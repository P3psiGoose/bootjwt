...
/**
 * Our custom authorisation based on JSON Web Tokens
 */
@Slf4j
		...
		if (request.getServletPath().equals("/api/login") || request.getServletPath().equals("/api/token/refresh")) {
			...
			if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
				try {
					...
					DecodedJWT token = verifier.verify(encodedToken);

					String userName = token.getSubject(); // s. CustomAuthenticationFilter
					String[] roleNames = token.getClaim("roles").asArray(String.class); // s. CustomAuthenticationFilter

					Collection<SimpleGrantedAuthority> authorities = new ArrayList<>();
					Arrays.stream(roleNames).forEach(role -> authorities.add(new SimpleGrantedAuthority(role)));
